...
router.post("/get", async (req, res) => {
  ...
  for (let index = 0; index < initlenght; index++) {
    let product =
      initdata.data.products[Object.keys(initdata.data.products)[index]];
  ...
  var sql =
    "select product_name, sku, product_id, MAX(timestamp) as timestamp ,count(*) as total from orders where timestamp > ? group by sku ";
  var valuse = [[datastamp]];
  ...
  for (let element of sql_result) {
    ...
    if (!productIDs.includes(element)) productIDs.push(element.product_id);
  }
    ...
    .then((res) => {
      sql_result.forEach((element_res) => {
        if (res.data.products[element_res.product_id]) {
          ...
          if (
            element_res.total >
              res.data.products[element_res.product_id].stock.bl_5662 &&
            !selectedProducts.some(
              (item) =>
                item.product_id.toString() === element_res.product_id.toString()
            )
          ) {
      ...
      allProducts.forEach((element) => {
        if (res.data.products[element.product_id]) {
          let product_name =
            res.data.products[element.product_id].text_fields.name;
          ...
          var month =
            (a.getUTCMonth() + 1 < 10 ? "0" : "") + (a.getUTCMonth() + 1); //months from 1-12
          ...
          let buy_price = Number.parseFloat(
            res.data.products[element.product_id].text_fields.extra_field_5072
          ).toFixed(2);
          let margin = (((price - buy_price * 1.23) / price) * 100).toFixed(2);
          let vendor =
            res.data.products[element.product_id].text_fields.extra_field_4240;
      ...
      selectedProducts.forEach((element) => {
        if (res.data.products[element.product_id]) {
          ...
          let isBundle =
            res.data.products[element.product_id].text_fields.extra_field_4690;

          if (isBundle != null && isActive != null) {
            if (
              isActive.toLowerCase() === "tak" &&
              isBundle.toLowerCase() === "nie"
            ) {
              let product_name =
                res.data.products[element.product_id].text_fields.name;
              let sku = res.data.products[element.product_id].sku;
              ...
              let in_stock =
                res.data.products[element.product_id].stock.bl_5662;
              let price = res.data.products[element.product_id].prices[4494];
              let buy_price = Number.parseFloat(
                res.data.products[element.product_id].text_fields
                  .extra_field_5072
              ).toFixed(2);
              let margin = (((price - buy_price * 1.23) / price) * 100).toFixed(
                2
              );
              let vendor =
                res.data.products[element.product_id].text_fields
    ...
    .then((response) => {
      ...
      connection.query(sql, function (err, result) {
        ...
        for (let index = 0; index < response.data.orders.length; index++) {
          ...
          for (
            let i = 0;
            i < response.data.orders[index].products.length;
            i++
          ) {
            ...
            for (let index = 0; index < quantity; index++) {
              if (!data.includes(orderID)) {
                ...
                var sql =
                  "INSERT INTO orders (orderID,product_id,product_name, sku, timestamp ) VALUES ( ?)";
                var valuse = [
                  [orderID],
                  [productID],
                  [productName],
...
function sendMail(attachments) {
  ...
  var month =
    (dateObj.getUTCMonth() + 1 < 10 ? "0" : "") + (dateObj.getUTCMonth() + 1); //months from 1-12
  ...
  let newdate =
    day + "." + month + "." + year + " godzina: " + hours + ":" + minutes;
...
function createPDF(alias, data, allFilenames) {
  ...
  let newdate =
    day + "_" + month + "_" + year + "_" + hours + "_" + minutes + "_" + alias;
  ...
  const tableArray = {
    headers: [
      ...
      {
        label: "Cena sprzedaży brutto (zł)",
        align: "center",
        headerAlign: "center",
      },
      {
  ...
  doc.table(tableArray, {
    ...
    prepareRow: (row, indexColumn, indexRow, rectRow) => {
      ...
      indexColumn === 0 &&
        doc.addBackground(rectRow, indexRow % 2 ? "#7f827e" : "#e0e6df", 0.15);
    },
  });
  doc.end();
}
      ...
      axios
        ...
        .then((response) => {
          ...
          con.query(sql, function (err, result) {
            ...
            for (let index = 0; index < response.data.orders.length; index++) {
              var timestamp = response.data.orders[index].date_confirmed;
              var orderID = Number.parseInt(
                response.data.orders[index].order_id
              );
              ...
              for (
                ...
                var productID =
                  response.data.orders[index].products[i].product_id;
                var productName = response.data.orders[index].products[i].name;
                var sku = response.data.orders[index].products[i].sku;
                var quantity = response.data.orders[index].products[i].quantity;
...
async function getLastPurchase(product_id, con) {
  var sql =
    "SELECT MAX(timestamp) as timestamp from orders where product_id = ? ";
  var valuse = [[product_id]];
