...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (
      orderPackagesDetails.packages_history &&
      orderPackagesDetails.packages_history.length > 0
    ) {
      for (
        let index = 0;
        index < Object.keys(orderPackagesDetails.packages_history).length;
        index++
      ) {
        let objindex = Object.keys(orderPackagesDetails.packages_history)[
          index
        ];
        for (
          let inner_index = 0;
          inner_index < orderPackagesDetails.packages_history[objindex].length;
          inner_index++
        ) {
          const element =
            orderPackagesDetails.packages_history[objindex][inner_index];
        }
      }
    } else {
      console.log("ELSE");
    ...
    for (let i = 0; i < orderPackages.packages.length; i++) {
      ...
      // }
      orderPackages.packages[i].status =
        trackingStatuses[orderPackages.packages[i].tracking_status];
      orderPackages.packages[i].package_number =
        orderPackages.packages[i].courier_package_nr;
    }

    let insuranceValue = getRawInsuranceValue(orderDetails);
    res.render("/media/data/kubart_sr/views/shipments.ejs", {
    ...
    return res.status(400).send({
      message:
        error.message +
        "     ->Sprawdź czy zamówienie jest potwierdzone w systemie baselinker",
    });
  }
});
