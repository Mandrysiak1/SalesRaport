...
async function getOrderPackagesDetails(orderIDs) {
  ...
  resp = await axios.post("https://api.baselinker.com/connector.php", data, {
    headers: {
      "X-BLToken": process.env.BASELINKER_API_KEY,
      "Content-Type": "multipart/form-data",
    },
...
function getDefaultShipmentMethod(orderDetails) {
  ...
  if (orderDetails.orders[0].order_source === "empik") {
    ...
    if (
      orderDetails.orders[0].delivery_method ===
        "Kurier InPost, Płatność z góry,Przesyłka kurierska" ||
      orderDetails.orders[0].delivery_method ===
        "Kurier InPost, Płatność przy odbiorze,Przesyłka kurierska pobraniowa"
    ) {
      return { tab: "inpost", value: "Przesyłka kurierska standardowa" };
    } else if (
      orderDetails.orders[0].delivery_method ===
      "Paczkomaty InPost, Płatność z góry,Przesyłka, Paczkomat płatność z góry"
    ) {
      return {
        tab: "paczkomaty",
        value: "Paczkomaty 24/7 - Przesyłka standardowa",
    ...
    if (
      orderDetails.orders[0].delivery_method === "Allegro Kurier DPD" ||
      orderDetails.orders[0].delivery_method ===
      ...
      orderDetails.orders[0].delivery_method === "Allegro Kurier UPS" ||
      orderDetails.orders[0].delivery_method ===
      ...
      orderDetails.orders[0].delivery_method === "Allegro Paczkomaty InPost"
    ) {
    ...
    if (
      orderDetails.orders[0].delivery_method ===
      "Przesyłka kurierska InPost Kurier Standard"
    ) {
      return { tab: "inpost", value: "Przesyłka kurierska standardowa" };
    } else if (
      orderDetails.orders[0].delivery_method ===
      "Przesyłka kurierska InPost Kurier Standard-Płatność za pobraniem"
    ) {
      return { tab: "inpost", value: "Przesyłka kurierska standardowa" };
    } else if (
      orderDetails.orders[0].delivery_method ===
